...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device: str = "cpu", wandb_logger=None) -> None:
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        for it in range(start_it, total_it):
            ...
            print (f"[INFO] Iteration {it}/{total_it} | Collect Time: {collect_time:.4f}s | Learn Time: {learn_time:.4f}s")
            ...
            if self.wandb_logger is not None:
                ...
                if self.cfg["algorithm"].get("rnd_cfg"):
                    log_dict["rnd_weight"] = self.alg.rnd.weight
                if loss_dict is not None:
                    log_dict.update(loss_dict)
                self.wandb_logger.log(log_dict, step=it)
                #Save model
                if it % 100 + 1 == 0:
                # if it % self.cfg["save_interval"] + 1 == 0:
                    self.wandb_logger.save(os.path.join(self.logger.log_dir, f"model_{it}.pt"))  # type: ignore
